...
class CSVOptions(
  ...
  val timestampFormats: Array[FastDateFormat] = parameters.get("timestampFormats").getOrElse("yyyy-MM-dd'T'HH:mm:ss.SSSXXX").split(",").map(s => s.trim).:+("yyyy-MM-dd'T'HH:mm:ss.SSSXXX").map(s => FastDateFormat.getInstance(s, timeZone, Locale.US))
